...
android {
     ...
     * the upgrade test cannot even describe what the previous release created — and the upgrade
 ...
 *
 * Two separate constraints, both hit only under a confined build sandbox — and both of which
 ...
 * — the resolver's own offline mechanism — maps the SDK level straight at the cached jar, so no
...
tasks.withType<Test>().configureEach {
    doFirst {
        ...
        roboDeps.writeText(
            buildString {
                appendLine("# Generated by app/build.gradle.kts — do not edit, do not commit.")
                ...
                for ((version, source) in versions) {
                    ...
                    // The key is `DependencyJar.getShortName()` — `groupId:artifactId:version` —                     // and those colons MUST be backslash-escaped: in a .properties file a colon
    ...
    // migration tests anyway — real SQL semantics rather than an emulated subset.
